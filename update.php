...
try {
    ...
    // 動的に変わる値をプレースホルダ(:id)に置き換えたsql分をあらかじめ用意する
        ...
        <article class="registration">
            ...
            <form action="update.php?id=<?= $_GET['id'] ?>" method="post" class="registration-form">
                <div>
                    ...
                    <input type="number" name="price" value="<?= $product['price'] ?>"maxlength="50" required>

                    <label for="stock_quantity">在庫数</label>
                    <input type="number" name="stock_quantity" value="<?= $product['stock_quantity'] ?>"maxlength="50" required>
                    ...
                    <select name="vendor_code" required>
                        ...
                        <?php
                        // 配列の中身を順番に取出し、セレクトボックスの選択肢として出力する
                        // echoは定型文と思ってよい
                        foreach ($vendor_codes as $vendor_code) {
                       // もし変数$vendor_codeが商品の仕入れ先コードの値と一致していればSELECT属性をつけて初期値にする
                ...
                <button type="submit" class="submit-btn" name="submit" value="creat">編集</button>
            </form>
        </article>
    </main>
    <footer>
